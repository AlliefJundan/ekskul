<x-layout>
    <div class="container mx-auto mt-6">
        ...
        <p class="text-gray-600 font-bold">Bulan:
            {{ DateTime::createFromFormat('!m', $bulan) ? DateTime::createFromFormat('!m', $bulan)->format('F') : 'Tidak Valid' }}
        </p>
                            ...
                            @foreach ($rekapAbsen as $index => $rekap)
                                <tr class="text-center">
                                    ...
                                    <td class="border border-gray-300 px-4 py-2 font-semibold">
                                        {{ $rekap->nama ?? ($rekap->username ?? 'Tidak Diketahui') }}</td>
                                    <td class="border font-bold text-lg border-gray-300 px-4 py-2">
                                        {{ $rekap->konfirmasi ?? 0 }}</td>
                                    <td class="border font-bold text-lg border-gray-300 px-4 py-2">
                                        {{ $rekap->hadir ?? 0 }}
                                    </td>
                                    <td class="border font-bold text-lg border-gray-300 px-4 py-2">
