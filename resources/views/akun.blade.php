                    ...
                    <tbody>
                        @foreach ($akun as $index => $user)
                            <tr
                                ...
                                <td class="p-2 font-bold text-gray-800 border border-gray-300">{{ $user->username }}
                                </td>
                ...
                <table class="w-full text-sm text-left text-white">
                    <thead class="bg-gray-100 text-gray-800">
                        <tr>
                            ...
                            <th class="px-4 py-2 flex justify-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($akun as $user)
                            <tr
                                class="border-b border-gray-600 bg-gray-150 {{ $loop->odd ? 'bg-maroon-600' : 'bg-maroon-700' }}">
                                <td class="px-4 py-2">
                                    <div class="font-bold">{{ $user->nama }}</div>
                                    <div class="text-xs text-gray-300">Username: {{ $user->username }}</div>
                                </td>
                                <td class="px-4 py-2">
                                    <span
                                        class="inline-block px-2 py-1 rounded {{ $user->role === 'admin' ? 'bg-white text-red-700 font-bold' : 'bg-white text-gray-700 font-semibold' }}">
                                        {{ $user->role }}
                                    </span>
                                </td>
                                <td class="px-4 py-2 space-y-1">
                                    <button
                                        onclick="openUbahModal('{{ $user->id_user }}', '{{ $user->username }}', '{{ $user->nama }}', '{{ $user->role }}')"
                                        class="w-full px-2 py-1 text-white bg-blue-500 rounded hover:bg-blue-600">Ubah</button>

                                    <button data-modal-target="#detailModal-{{ $user->id_user }}"
                                        class="w-full px-2 py-1 text-white bg-green-500 rounded hover:bg-green-600">Detail</button>
                                    ...
                                    <form action="{{ route('akun.destroy', $user->id_user) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="w-full px-2 py-1 text-white bg-red-500 rounded hover:bg-red-600">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                        ...
                        <div class="grid items-center grid-cols-3 gap-2 mb-3">
                            ...
                            <div class="relative w-full col-span-2">
                                <input type="password" id="passwordInput" name="password"
                                    placeholder="Masukan Password" class="w-full p-2 text-gray-800 rounded" required>
                ...
                <div class="relative p-6 text-white bg-blue-800 rounded-lg shadow-lg w-96">
                    ...
                    <form id="formUbahAkun" method="POST">
                        ...
                        <div class="grid items-center grid-cols-3 gap-2 mb-3">
                            <label class="col-span-1 font-bold text-right">Password :</label>
                            <div class="relative w-full col-span-2">
                                <input type="password" id="ubahPassword" name="password"
                                    placeholder="Masukan Password" class="w-full p-2 text-gray-800 rounded">
