...
<div x-data="{ open: false }">
    ...
    <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
        ...
        <div
            class="bg-gray-200 text-gray-700 rounded-lg p-4 shadow-lg relative w-11/12 max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg">
            <button @click="open = false" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 text-xl">
                ✖
            </button>
            <h2 class="text-xl text-black font-bold mb-4 flex justify-center">{{ $title }}</h2>
            {{ $slot }}
        </div>
    </div>
</div>
